 ...
 * 
 * Run with: node scripts/tests/kilocode-config-integration.test.mjs
 */
...
test('should read config file from disk', () => {
  ...
  assertEqual(config.profiles[0].provider, 'kilocode');
  assertEqual(config.profiles[0].kilocodeToken, 'test-api-key-12345');
...
test('should generate valid config for openrouter provider', () => {
  const config = generateKilocodeConfig({
  ...
  const configJson = JSON.stringify(config, null, 2);
  const openrouterConfigPath = join(configDir, 'openrouter-config.json');
  writeFileSync(openrouterConfigPath, configJson);
  
  const readConfig = JSON.parse(readFileSync(openrouterConfigPath, 'utf-8'));
  assertEqual(readConfig.profiles[0].provider, 'openrouter');
  assertEqual(readConfig.profiles[0].openRouterApiKey, 'sk-or-test-key');
  assertEqual(readConfig.profiles[0].openRouterModelId, 'anthropic/claude-3-5-sonnet');
});
...
test('should simulate GitHub Action config generation', () => {
